...
# HTML templates (Assuming these files are correctly defined and available)
try:
    from htmlTemplates import css, bot_template, user_template
except ImportError:
    # Fallback definitions for completeness
    css = """
    <style>
    ...
    .source-info { font-size: 0.8em; color: #bbb; margin-top: 10px; border-top: 1px solid #555; padding-top: 5px; }
    </style>
    """
    bot_template = """
    <div class="chat-message bot">
        <div class="avatar"><img src="https://i.ibb.co/cN0nmSj/android-chrome-512x512.png" style="max-height: 78px; max-width: 78px; border-radius: 50%; object-fit: cover;"></div>
        <div class="message">{{MSG}}</div>
    </div>
    """
    user_template = """
    <div class="chat-message user">
        <div class="avatar"><img src="https://i.ibb.co/F5Gz1W5/1946429.png" style="max-height: 78px; max-width: 78px; border-radius: 50%; object-fit: cover;"></div>
        <div class="message">{{MSG}}</div>
    </div>
    """

# CUSTOM PROMPT TEMPLATE FIX
CUSTOM_PROMPT_TEMPLATE = """Use the following pieces of context to answer the user's question.
If you don't know the answer, just say that you don't know, don't try to make up an answer.
Your final answer must be a clear, concise, and helpful response based *only* on the provided context.
DO NOT include any preceding phrases like 'Helpful Answer:', 'Question:', or 'Final Answer:' in your response.
Your response should only be the answer itself.
...
def get_llm_endpoint(token):
    ...
    return HuggingFaceEndpoint(
        ...
        temperature=0.5,
        max_new_tokens=512,  # Fixed Repetition
...
def get_chain(vectorstore, token, with_memory=True):
    ...
    # 🌟 FIX: Create a custom prompt template object
    custom_qa_prompt = PromptTemplate.from_template(CUSTOM_PROMPT_TEMPLATE)
    
    chain = ConversationalRetrievalChain.from_llm(
        llm=llm,
        retriever=vectorstore.as_retriever(),
        memory=memory,
        # 🌟 FIX: Pass the custom prompt to stop internal chatter
